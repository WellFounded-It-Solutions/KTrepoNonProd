...
class FlowPlayerParserTest {

    @Test
    fun testParsePlayerObject() {
        val parser = Xml.newPullParser()
        parser.setInput(ResourceHelper.getResourceString("flow_player.xml").byteInputStream(), "UTF-8")
        parser.next()

        val list = FlowPlayerParser().parse(parser)
        (list.firstOrNull() as FlowPlayerItem).let {
            assertEquals("x-im/flowplayer", it.type)
