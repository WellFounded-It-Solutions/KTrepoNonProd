...
import se.infomaker.iap.provisioning.permission.PermissionConfig
...
class PaywallFragmentGota : Fragment() {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        container?.context?.let { context ->
            return ConstraintLayout(context).apply {
                ...
                val paywallAnchor = LayoutInflater.from(context)
                    .inflate(R.layout.paywall_anchor, this, false)
                addView(paywallAnchor)

                val paywallHeaderView: View
                if (headerLayout != 0) {
                    paywallHeaderView = LayoutInflater.from(context)
                        .inflate(headerLayout, this, false)
                    addView(paywallHeaderView)
                } else {
                    paywallHeaderView = LayoutInflater.from(context)
                        .inflate(R.layout.empty_paywall_header, this, false)
                    addView(paywallHeaderView)
                }
                ...
                addView(ScrollView(context).apply {
                    ...
                    id = R.id.paywall
                    val paywallView = LayoutInflater.from(context)
                        .inflate(R.layout.paywall_fragment_gota, this, false)
                    addView(paywallView)
                })
                ...
                constraintSet.connect(
                    ConstraintSet.PARENT_ID,
                    ConstraintSet.START, paywallHeaderView.id,
                    ConstraintSet.START, 0)
                constraintSet.connect(
                    ConstraintSet.PARENT_ID,
                    ConstraintSet.END, paywallHeaderView.id,
                    ConstraintSet.END, 0)
                constraintSet.connect(paywallHeaderView.id,
                    ConstraintSet.TOP,
                    R.id.paywallAnchor,
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val domain = config?.provisioning?.customerServiceUrl
            ...
            when (it) {
                ViewState.PURCHASE -> {
                    if (model.loginEnabled() && model.purchasesEnabled()) {
                        ...
                        inflated.findViewById<ThemeableMaterialButton>(R.id.showLogin).setText(R.string.provisioning_customer_service_button)

                        inflated.isSubscriptionIn = LoginStatus.LOGGED_IN == LoginStatus.LOGGED_IN
