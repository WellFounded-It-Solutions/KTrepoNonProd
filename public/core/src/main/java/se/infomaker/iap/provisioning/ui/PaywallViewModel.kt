...
class PaywallViewModel(application: Application) : AndroidViewModel(application) {
    ...
    init {
        ...
        garbage.add(Observable.combineLatest(productProviders) { combined ->
            ...
            hasValidProduct.accept(provisioningManager.canStartAppWithProducts(it))
        })
    ...
    private fun updateProgressViewVisibility() {
        var visible = false
        val currentLoginStatus = loginManager?.getLoginStatus()
        visible = visible || currentLoginStatus == LoginStatus.IN_PROGRESS
        billingManager?.apply {
    ...
    private fun needsLinking(): Boolean {
        ...
        if (provisioningManager.canStartAppWithProducts(availableProducts)) {
    ...
    private fun linkAccountToPurchase(purchase: Purchase, onSuccess: () -> Unit, onError: (Throwable) -> Unit) {
        ...
        updateProgressViewVisibility()
        link = {
            provisioningManager.linkAccount()
                    .subscribeOn(Schedulers.io())
                    .observeOn(AndroidSchedulers.mainThread())
                    .subscribe { _: FunctionResult<LinkAccountResponse>?, error: Throwable? ->
    ...
    fun loginWithCredential(context: Context, credential: Credential) {
        viewState.value = ViewState.IN_PROGRESS
        provisioningManager.loginManager()?.login(credential.username, credential.password, {
            viewState.value = ViewState.PURCHASE
        }, { error ->
